...
unsigned int loadCubeMapTexture(const char *directory,std::vector<std::string>& faces);
...
//    glFramebufferRenderbuffer(GL_FRAMEBUFFER,GL_DEPTH_STENCIL_ATTACHMENT,GL_RENDERBUFFER,RBO);
    ...
    unsigned int skyboxTexture = loadCubeMapTexture("./skybox",faces);
    ...
    context.skyboxVAO = skyboxVAO;
    context.skyboxShader = &skyboxShader;
    context.skyboxTexture = skyboxTexture;
...
unsigned int loadCubeMapTexture(const char *directory,std::vector<std::string>& faces) {
    ...
    for (int i = 0; i < faces.size(); ++i) {
        ...
        basePath = basePath + "/" + faces[i];
        unsigned char * data = stbi_load(basePath.c_str(),&width,&height,&nrChannels,0);
        if (!data) {
